# 学習計画: kjr-browser (フルスクラッチ版・アーカイブ)

> **注**: プロジェクトは Tauri ベースの軽量ブラウザ開発に方針転換した (現行の計画は [README](../README.md) を参照)。
> このドキュメントはレンダリングエンジンをフルスクラッチで実装する当初の学習計画で、
> ブラウザ内部の仕組みを学ぶ資料として残している。
