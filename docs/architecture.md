# アーキテクチャ: レイヤー構成とブラウザ理解の対応 (シェル編)

> **注**: このドキュメントは Tauri 製ブラウザシェル (src-tauri/ + ui/) の解説。
> プロジェクトの本線はフルスクラッチのレンダリングエンジン ([engine/](../engine/),
> 計画は [learning-plan.md](learning-plan.md)) に戻った。ここに書かれた
> 「ブラウザプロセス側」の構造理解はエンジン学習の前提知識としてそのまま有効。
